...
void dfs(vector<vector<char>>&grid, int r, int c, vector<vector<bool>>&visited){
    ...
    vector<int> rNbr = { 0, 1, 1, 1, 0, -1,-1,-1}; /// assume as X axis
    vector<int> cNbr = { 1, 1, 0, -1,-1,-1, 0, 1 };///assume ay Y axis

    visited[r][c] = true;

    ///travers all Land(L) negisbour and include it's Island area
    for(int k=0; k<8; ++k){ /// k<8 bcz every land(L) has at most 8 neighbour
...
int main(){
    ...
    cout<<countIslands(grid)<<endl; // 3 island
